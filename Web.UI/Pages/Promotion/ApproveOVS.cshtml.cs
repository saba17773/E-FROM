    ...
    public class ApproveOVSModel : PageModel
    {
        ...
        private async Task InitialDataAsync(int id, int tid, int level)
        {
            ...
            using (var unitOfWork = new UnitOfWork(_databaseContext.GetConnection()))
            {
                ...
                Promotion_OVS = promotion;

                Promotion_OVS.RequestNumber = promotion.RequestNumber;
                Promotion_OVS.RequestDate = promotion.RequestDate;
                TypeOfProduct = typeOfProduct.ByCode + " (" + typeOfProduct.ByName + ") ";
                Promotion_OVS.Pattern = promotion.Pattern;
                Promotion_OVS.CustomerName = promotion.CustomerName;
                TypeOf = typeOf.PromotionType;
                Promotion_OVS.TypeOfRemark = promotion.TypeOfRemark;
                CustomerGroup = customerGroup.GroupName;
                Promotion_OVS.CustomerGroupRemark = promotion.CustomerGroupRemark;
                Promotion_OVS.PaymentType = promotion.PaymentType;
                Promotion_OVS.FromDate = promotion.FromDate;
                Promotion_OVS.ToDate = promotion.ToDate;
                TypeFrom = typeFrom.PromotionFrom;
                Promotion_OVS.TypeFromRemark = promotion.TypeFromRemark;
                Promotion_OVS.Objective = promotion.Objective;
                // Promotion_OVS.SalesPresentBath = promotion.SalesPresentBath;
                // Promotion_OVS.SalesForecastBath = promotion.SalesForecastBath;
                // Promotion_OVS.SalesChangeBath = promotion.SalesChangeBath;
                Promotion_OVS.SalesRemarkBath = promotion.SalesRemarkBath;
                // Promotion_OVS.SalesPresentQty = promotion.SalesPresentQty;
                // Promotion_OVS.SalesForecastQty = promotion.SalesForecastQty;
                // Promotion_OVS.SalesChangeQty = promotion.SalesChangeQty;
                Promotion_OVS.SalesRemarkQty = promotion.SalesRemarkQty;
                // Promotion_OVS.BudgetPresent = promotion.BudgetPresent;
                // Promotion_OVS.BudgetForecast = promotion.BudgetForecast;
                // Promotion_OVS.BudgetChange = promotion.BudgetChange;
                Promotion_OVS.BudgetRemark = promotion.BudgetRemark;
                ...
                Promotion_OVS.GetDiscount = promotion.GetDiscount;
                Promotion_OVS.GetPoint = promotion.GetPoint;
                Promotion_OVS.PromotionConditions = promotion.PromotionConditions;
                OVSLevel = level;
                IsFile = IsFile;
                ...
                var attachFileref = await unitOfWork.Promotion.GetFileByCCIdAsync(promotion.PromotionRef, nameof(RequestTypeModel.OVS));
        ...
        public async Task<IActionResult> OnPostAsync(int id, int tid, int level, string nonce)
        {
            try
            {
                ...
                using (var unitOfWork = new UnitOfWork(_databaseContext.GetConnection()))
                {
                            ...
                            if (promotion.PromotionRef > 0 && Regex.IsMatch(promotion.RequestNumber, "Excep?"))
                            {
                                ...
                                var sendNextEmail = _emailService.SendEmail(
                                    ...
                                    $@"
                                        ...
                                        <b>ชื่อเรื่อง : </b> {promotion.Pattern}<br/>
                                        <b>ชื่อลูกค้า : </b> {promotion.CustomerName}<br/>
                                        <b>Link เพื่อดำเนินการ : </b> <a href='{_configuration["Config:BaseUrl"]}/Promotion/ApproveOVS?id={promotion.Id}&tid={nextApproveTrans.Id}&level={level + 1}&nonce={nonceKey}'>คลิกที่นี่</a> <br/>
                    ...
                    // Upload file.
                    string basePath = $"wwwroot/files/promotion/ovs/{(int)promotion.Id}";
