...
{
    public class MarketSubgroupModel : PageModel
    {
        [TempData]
        ...
        public async Task<IActionResult> OnPostAsync()
        {
            try
            {
                ...
                var filePath = "";
                var rootPathUpload = "wwwroot/files/import/market_subgroup";
                ...
                {
                    HeaderRow = true
                }.Fetch<ImportTemplateMarketSubGroupExcelModel>(0);
                ...
                using (var unitOfWork = new UnitOfWork(_databaseContext.GetConnection("Import")))
                {
                    ...
                    var doCompanyList = t.Connection.Query<ClearOldDataParamsModel>(@"
                        ...
                        FROM TB_ImportTemplateMarketSubGroup
                    ...
                    foreach (var item in doCompanyList)
                    {
                        await t.Connection.ExecuteAsync(@"
                            DELETE FROM TB_ImportTemplateMarketSubGroup
                    ...
                    {
                        await t.Connection.InsertAsync(new ImportTemplateMarketSubgroup
                        {
                            Year = item.Year,
                    ...
                    return Redirect($"/Purchase/Import/MarketSubgroup");
                }
            }
            catch (Exception ex)
            {
