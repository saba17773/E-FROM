...
namespace Web.UI.Pages.S2E.Qtech.LABTest
{
    public class CreateModel : PageModel
    {
        ...
        public async Task<IActionResult> OnGetAsync(int RequestID, int AssessmentID)
        {
            try
            {
                ...
                using (var unitOfWork = new UnitOfWork(_databaseContext.GetConnection()))
                {
                    ...
                    var CheckRequest = LABTestHeadALL.Where(x => x.ASSESSMENTID == AssessmentID).FirstOrDefault();
                    ...
                    await GetData(RequestID, AssessmentID);
        ...
        public async Task GetData(int RequestID,int AssessmentID)
        {
            using (var unitOfWork = new UnitOfWork(_databaseContext.GetConnection()))
            {
        ...
        public async Task<IActionResult> OnPostAsync(int RequestID, int AssessmentID, string draft, string save)
        {
            ...
            try
            {
                using (var unitOfWork = new UnitOfWork(_databaseContext.GetConnection()))
                {
                    ...
                    var AddRMSampleID = AddRMSampleALL.Where(x=>x.REQUESTID == RequestID && x.ASSESSMENTID == AssessmentID)
                                                      .Select(s=>s.ID).FirstOrDefault();

                    var RMReqSampleHeadRepo = new GenericRepository<S2EMaterialRequestSampleHead_TB>(unitOfWork.Transaction);
                    var RMReqHeadALL = await RMReqSampleHeadRepo.GetAllAsync();
                    var RMReqHeadByAddRMSampleID = RMReqHeadALL.Where(x => x.ADDRMSAMPLEID == AddRMSampleID).FirstOrDefault();

                    if (RMReqHeadByAddRMSampleID == null)
                    {
                        //UPDATE RM ASSESSMENT
                        var RMAssessmentRepo = new GenericRepository<S2ERMAssessment_TB>(unitOfWork.Transaction);
                        var RMAssessmentByID = await RMAssessmentRepo.GetAsync(AssessmentID);
                        RMAssessmentByID.APPROVESTATUS = RequestStatusModel.Successfully;
                        RMAssessmentByID.COMPLETEBY = CreateBy;
                        RMAssessmentByID.COMPLETEDATE = RequestDate;
                        await RMAssessmentRepo.UpdateAsync(RMAssessmentByID);
                    }
                    else
                    {
                        //UPDATE 
                        var RMReqSampleHeadByID = await RMReqSampleHeadRepo.GetAsync(RMReqHeadByAddRMSampleID.ID);
                        RMReqSampleHeadByID.REQUESTSTATUS = RequestStatusModel.Successfully;
                        ...
                        var RMReqSampleLineByHeadID = RMReqSampleLineALL.Where(x => x.RMREQSAMID == RMReqSampleHeadByID.ID 
                                                                                 && x.APPROVESTATUS != 2);
                        foreach (var Line in RMReqSampleLineByHeadID)
                        {
                            var UpdateLine = await RMReqSampleLineRepo.GetAsync(Line.ID);

                            UpdateLine.COMPLETEBY = CreateBy;
                            UpdateLine.COMPLETEDATE = RequestDate;
                            await RMReqSampleLineRepo.UpdateAsync(UpdateLine);
                        }
                    }
                    ...
                    //INSERT LAB TEST Head TABLE
                    var LabTestHeadInsert = new S2ELABTestHead_TB 
                    {
                        REQUESTID = RequestID,
                        ASSESSMENTID = AssessmentID,
                    ...
                    var LABTestID = await LabTestHeadRepo.InsertAsync(LabTestHeadInsert);
                    ...
                    if (!string.IsNullOrEmpty(save))
                    {
                        ...
                        var AppTransByRequestID = await unitOfWork.S2EControl.GetApproveTransByLABID((int)LABTestID, (int)LABTestLineID, approvemasterid, ApproveGroupID);
            ...
            {
                AlertError = ex.Message;
                return Redirect("/S2E/Qtech/LABTest/" + RequestID + "/" + AssessmentID + "/Create");
            }
        }
    }
}
