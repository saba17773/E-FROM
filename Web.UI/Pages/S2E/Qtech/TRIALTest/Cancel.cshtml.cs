...
namespace Web.UI.Pages.S2E.Qtech.TRIALTest
{
    public class CancelModel : PageModel
    {
        ...
        public async Task<IActionResult> OnPostAsync(int TRIALID, int TRIALLINEID)
        {
            try
            {
                using (var unitOfWork = new UnitOfWork(_databaseContext.GetConnection()))
                {
                    ...
                    var AddRMLineRepo = new GenericRepository<S2EAddRawMaterialLine_TB>(unitOfWork.Transaction);
                    var AddRMLineALL = await AddRMLineRepo.GetAllAsync();
                    var AddRMLineByHeadID = AddRMLineALL.Where(x=>x.ADDRMID == ADDRMID);
                    ...
                    LABTestHeadByID.CANCELREMARK = CancelRemark;
                    RMReqHeadByID.CANCELREMARK = CancelRemark;
                    TrialTestHeadByID.CANCELREMARK = CancelRemark;

                    NewRequestByID.APPROVESTATUS = RequestStatusModel.Cancel;
                    RMAssessmentByID.APPROVESTATUS = RequestStatusModel.Cancel;
                    LABTestLineByID.APPROVESTATUS = RequestStatusModel.Cancel;
                    PCSampleByID.APPROVESTATUS = RequestStatusModel.Cancel;
                    foreach (var updatelineAddRM in AddRMLineByHeadID)
                    {
                        updatelineAddRM.APPROVESTATUS = RequestStatusModel.Cancel;
                        updatelineAddRM.COMPLETEDATE = DatetimeNow;
                        updatelineAddRM.COMPLETEBY = CancelBy;
                        await AddRMLineRepo.UpdateAsync(updatelineAddRM);
                    }
                    foreach (var updatelineRMReq in RMReqLinebyHeadID)
                    {
                        updatelineRMReq.APPROVESTATUS = RequestStatusModel.Cancel;
                        updatelineRMReq.COMPLETEDATE = DatetimeNow;
                        updatelineRMReq.COMPLETEBY = CancelBy;
                        await RMReqLineRepo.UpdateAsync(updatelineRMReq);
                    }
                    TrialTestLineByID.APPROVESTATUS = RequestStatusModel.Cancel;
                    ...
                    LABTestLineByID.COMPLETEBY = CancelBy;
                    PCSampleByID.COMPLETEDATE = DatetimeNow;
                    PCSampleByID.COMPLETEBY = CancelBy;
                    TrialTestLineByID.COMPLETEDATE = DatetimeNow;
                    TrialTestLineByID.COMPLETEBY = CancelBy;
            ...
            {
                AlertError = ex.Message;
                return Redirect($"/S2E/Qtech/TRIALTest/{TRIALID}/{TRIALLINEID}/Cancel");
            }
        }
    }
}
