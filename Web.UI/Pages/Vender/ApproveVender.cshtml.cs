...
{
    public class ApproveVenderModel : PageModel
    {
        [TempData]
        ...
        public async Task<IActionResult> OnPostAsync(int VendID, int TranID, string nonce, string email)
        {
            try
            {
                ...
                if (ApproveResult == 2 && ApproveRemark == null)
                {
                    ...
                    return Redirect($"/Vender/ApproveVender?VendID={VendID}&TranID={TranID}&nonce={nonce}&email={email}");
                }
                ...
                using (var unitOfWork = new UnitOfWork(_databaseContext.GetConnection()))
                {
                    ...
                    if (venderTable.CURRENTAPPROVESTEP == approveTransAll.ToList().Count && ApproveResult == 1)
                    {
                        ...
                        {
                            venderTable.APPROVESTATUS = RequestStatusModel.Complete;
                            ...
                            var sendCompleteEmail = _emailService.SendEmail(
                                    $"Subject : {venderTable.REQUESTCODE} / {venderTable.VENDCODE} /  การแจ้งขอ {AppGroup.DESCRIPTION} ผ่านการอนุมัติเรียบร้อยแล้ว",
                                    $@"
                                     <b>การแจ้งขอ Create {AppGroup.DESCRIPTION} ผ่านการอนุมัติเรียบร้อยแล้ว</b> <br/><br/>
                                     <b>เลขที่คำขอ   {venderTable.REQUESTCODE} </b> <br/>
                                     <b>รหัสผู้ขาย    {venderTable.VENDCODE} </b> <br/>
                                     <b>ชื่อผู้ขาย    {venderTable.VENDNAME} </b> <br/>
                                       <br/><br/><br/>
                                 ",
                                    emailcomplete,
                                    new List<string> { }
                    ...
                    else if (ApproveResult == 2 && ApproveRemark != null)
                    {
                        ...
                        var sendRejectEmail = _emailService.SendEmail(
                                $"Subject : {venderTable.REQUESTCODE} / Reject {AppGroup.DESCRIPTION} / ร้องขอเพื่อสร้างผู้ขายรายใหม่ในระบบ {vendercode}",
                                $@"
                    ...
                    else
                    {
                        ...
                        /*var vendtransRepo = new GenericRepository<VenderApproveTrans_TB>(unitOfWork.Transaction);
                        var vendtransALL = await vendtransRepo.GetAllAsync();

                        var countRequest = vendtransALL.Where(x => x.REQUESTID == venderTable.ID && x.APPROVEMASTERID == venderTable.APPROVEMASTERID && x.APPROVELEVEL == venderTable.CURRENTAPPROVESTEP).Count();
                        var subject = "";
                        var body = "";
                        ...
                        if (countRequest > 1)
                        {
                            subject = "Revise "+AppGroup.DESCRIPTION+" / ร้องขอเพื่อแก้ไขข้อมูลผู้ขาย ";
                            body = "ร้องขอเพื่อเพื่อแก้ไขข้อมูลผู้ขาย ";
                        }
                        else
                        {
                            subject = "Create " + AppGroup.DESCRIPTION + " / ร้องขอเพื่อสร้างผู้ขายรายใหม่ในระบบ ";
                            body = "ร้องขอเพื่อสร้างผู้ขายรายใหม่ในระบบ ";
                        }*/
                        // next approve trans
                        /*var nextApproveTrans = await unitOfWork.VenderControl.GetApproveTransByLevel(venderTable.ID, 
                                                venderTable.CURRENTAPPROVESTEP, approvemasterid);*/
                        ...
                        foreach (var i in nextLevel)
                        {
                            ...
                            //Subject : 2020/06/00001/ Create /ร้องขอเพื่อสร้างผู้ขายรายใหม่ในระบบ/ V-XXXX 
                            var sendNextEmail = _emailService.SendEmail(
                                   $"Subject : {venderTable.REQUESTCODE} / Create {AppGroup.DESCRIPTION} / ร้องขอเพื่อสร้างผู้ขายรายใหม่ในระบบ / {venderTable.VENDCODE}",
                                   $@"
                                    <b> ร้องขอเพื่อสร้างผู้ขายรายใหม่ในระบบ {venderTable.VENDCODE} </b><br/>
                                    <b> ผู้ร้องขอ  </b> {EmpFullName}<br/><br/>
                                    <b> รหัสผู้ขาย </b> {venderTable.VENDCODE}<br/>
                                    <b> ชื่อผู้ขาย  </b> {venderTable.VENDNAME}<br/>
                                    <b> ที่อยู่     </b> {venderTable.ADDRESS}<br/>
                                    <b> ชื่อผู้ติดต่อ </b> {venderTable.CONTACTNAME}<br/>
                                ...
                                ",
                                    //<b>Link เพื่อดำเนินการ:</b> <a href='{_configuration["Config:BaseUrl"]}/Vender/ApproveVender?VendID={VendID}&TranID={nextApproveTrans.ID}&nonce={nonceKey}'>คลิกที่นี่</a> <br/>
