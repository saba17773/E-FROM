                ...
                using (var unitOfWork = new UnitOfWork(_databaseContext.GetConnection()))
                {
                    ...
                    if (venderTable.CURRENTAPPROVESTEP == approveTransAll.ToList().Count && ApproveResult == 1)
                    {
                        ...
                        else
                        {
                            ...
                            emailcomplete.Add(UserALL.Email);
                            var sendCompleteEmail = _emailService.SendEmail(
                                     //Subject : 2020/27/00009 / V-4765 /ดำเนินการ Active เรียบร้อยแล้ว
                                     $"Subject : {venderTable.REQUESTCODE} / {venderTable.VENDCODE_AX} / ดำเนินการ {isactivesub} เรียบร้อยแล้ว",
                                    $@"
                                     <b>ดำเนินการ {isactivesub} เรียบร้อยแล้ว</b> <br/><br/>
                                     <b>เลขที่คำขอ   {venderTable.REQUESTCODE} </b> <br/>
                    ...
                    else if (ApproveResult == 2 && ApproveRemark != null)
                    {
                        ...
                        if (venderTable.ISACTIVE == 0)
                        {
                            isactivesub = " ขอระงับผู้จำหน่ายในระบบ ";
                        }
                        else
                        {
                            isactivesub = " ขอยกเลิกการระงับผู้จำหน่าย ";
                        }
                        var sendRejectEmail = _emailService.SendEmail(
                                $"Subject : {venderTable.REQUESTCODE} / Reject / ร้องขอเพื่อ{isactivesub} {vendercode}",
                                $@"
                                    <b> ร้องขอเพื่อ{isactivesub} {vendercode} </b><br/>
                                    ...
                                    <b style='color:red'>สาเหตุที่{isactivesub}  </b> {venderTable.REMARK}<br/>
                                    <br/><br/>
                            ...
                            if (vendTbTemp3 != null)
                            {
                                var sendNextEmail = _emailService.SendEmail(
                                   $"Subject : {venderTable.REQUESTCODE} {subject} {venderTable.VENDCODE_AX} ",
                                    ...
                                    <b> ชื่อผู้ขาย  </b> {venderTable.VENDNAME}<br/>
                                    <b> ที่อยู่     </b> {vendTbTemp3.ADDRESS_TEMP}<br/>
                                    <b> ชื่อผู้ติดต่อ </b> {vendTbTemp3.CONTACTNAME_TEMP}<br/>
                                    <b> โทรศัพท์  </b> {vendTbTemp3.TEL_TEMP}<br/>
                                    <br/><br/><br/>
                                    <b style='color:red'>สาเหตุที่{body}  </b> {vendTbTemp3.REMARK_TEMP}<br/>
                                    <br/><br/><br/>
                                    ...
                                    ",
                                    //<b>Link เพื่อดำเนินการ:</b> <a href='{_configuration["Config:BaseUrl"]}/Vender/ApproveVender_isActive?VendID={VendID}&TranID={nextApproveTrans.ID}&nonce={nonceKey}'>คลิกที่นี่</a> <br/>
