    ...
    public class ReviseModel : PageModel
    {
        ...
        public async Task<IActionResult> OnGetAsync(int id)
        {
            ...
            try
            {
                ...
                {
                    var Create = _authService.GetClaim().UserId;

                    var vendTbRepo = new GenericRepository<VenderTable_TB>(unitOfWork.Transaction);
                    var vendTbALL = await vendTbRepo.GetAllAsync();
                    var vendTb = vendTbALL.Where(x => x.ID == id && x.CREATEBY == Create).FirstOrDefault();
                    var vendTb2 = vendTbALL.Where(x => x.ID == id && x.UPDATEBY == Create).FirstOrDefault();
                    
                    if (vendTb == null && vendTb2 == null)
                    {
        ...
        public async Task<List<SelectListItem>> GetProvincesMaster()
        {
            using (var unitOfWork = new UnitOfWork(_databaseContext.GetConnection()))
            {
                var provRepo = new GenericRepository<ProvincesMaster_TB>(unitOfWork.Transaction);

                var provALL = await provRepo.GetAllAsync();
        ...
        public async Task<List<SelectListItem>> GetDistrictsMaster(int provin)
        {
            using (var unitOfWork = new UnitOfWork(_databaseContext.GetConnection()))
            {
                var disRepo = new GenericRepository<DistrictsMaster_TB>(unitOfWork.Transaction);

                var disALL = await disRepo.GetAllAsync();

                return disALL
                    .Where(x => x.ProvinceId == provin)
                    .Select(x => new SelectListItem
                    {
                        Value = x.Id.ToString(),
                        Text = x.NameInThai
        ...
        public async Task<JsonResult> OnGetDistrictByProvinceAsync(int provinceId)
        {
            using (var unitOfWork = new UnitOfWork(_databaseContext.GetConnection()))
            {
                var districtRepo = new GenericRepository<DistrictsMaster_TB>(unitOfWork.Transaction);
        ...
        public async Task<JsonResult> OnGetZipcodeByProvinceAsync(int subdistrictId)
        {
            using (var unitOfWork = new UnitOfWork(_databaseContext.GetConnection()))
            {
                var subDistrictRepo = new GenericRepository<SubDistrictTable>(unitOfWork.Transaction);
                ...
                //return new JsonResult(subDistrictAll.Where(x => x.Id == subdistrictId).ToList());
                return new JsonResult(subDistrictAll.Where(x => x.Id == subdistrictId));
            }
        }
        ...
        public async Task<IActionResult> OnPostAsync(int id)
        {
            ...
            try
            {
                using (var unitOfWork = new UnitOfWork(_databaseContext.GetConnection()))
                {
                    ...
                    int approvemasterid_old = venderTable.APPROVEMASTERID;

                    var vendgroupid = venderTable.VENDPROCESSID;
                    var approveMapRepo = new GenericRepository<VenderApproveMapping_TB>(unitOfWork.Transaction);
                    var approveMapALL = await approveMapRepo.GetAllAsync();
                    var approveMapByCreateBy = approveMapALL.Where(x => x.CreateBy == CreateBy &&
                                                              x.APPROVEGROUPID == vendgroupid &&
                                                              x.DATAAREAID == DataAreaId).FirstOrDefault();

                    var approvemasterid = approveMapByCreateBy.APPROVEMASTERID;
                    var vendprocessid = approveMapByCreateBy.APPROVEGROUPID;

                    if (venderTable.VENDCODE_AX != null && venderTable.COMPLETEDATE != null)
                    {
                        venderTable.CURRENTAPPROVESTEP = 1;
                        venderTable.APPROVESTATUS = 3;
                        venderTable.SENDMAILSUCCESS = 0;
                        venderTable.ISREVISE = 2;

                        //approvemasterid = approvemasterid_old;
                        ...
                        var VenderTrnasByRequestID = await unitOfWork.VenderControl.GetApproveTranIsCurrentByRequestID(id, approvemasterid_old);
                        ...
                        if (DocRef6.ToString() == "True")
                        {
                            ...
                            {
                                throw new Exception("กรุณาใส่เอกสารผู้ขาย อื่นๆ");
                            }
                            else
                            {
                                doc6 = 6;
                        ...
                        foreach (var i in approveTransLevel1)
                        {
                            ...
                            var sendRevisemail = _emailService.SendEmail(
                                  ...
                                  $@"
                                    ...
                                    <br/><br/><br/>
                                    <b style='color:red'> สาเหตุที่{body} </b> {Remark}
                                    <br/><br/><br/>
                                    <b style='color:black'> ชื่อผู้ทำการอนุมัติ </b> {ApproveBy}<br/></br/>
                                    <b>Link เพื่อดำเนินการ: </b> <a href='{_configuration["Config:BaseUrl"]}/Vender/VenderRequestList?Email={i.EMAIL}'>คลิ๊กที่นี่</a> <br/>
                                ",
                                   //<b style='color:black'>Link ���ʹ��Թ���:</b> <a href='{_configuration["Config:BaseUrl"]}/Vender/ApproveVender_isActive?VendID={id}&TranID={approveTransLevel1.ID}&nonce={nonceKey}'>��ԡ�����</a>
                    ...
                    {
                        //update vender table
                        venderTable.VENDIDNUM = VenderIDNum;
                        venderTable.VENDNAME = VenderName;
                        venderTable.ADDRESS = Address;
                        venderTable.CONTACTNAME = ContactName;
                        venderTable.TEL = Telephone;
                        venderTable.FAX = Fax;
                        venderTable.WEBSITE = Website;
                        venderTable.EMAIL = Email;
                        venderTable.VENDGROUPID = VenderGroup;
                        venderTable.VENDTYPEID = VenderType;
                        venderTable.CURRENCY = Currency;
                        venderTable.PRODTYPEID = ProductType;
                        venderTable.PAYMTERMID = Paymterm;
                        venderTable.REMARK = Remark;
                        venderTable.CURRENTAPPROVESTEP = 1;
                        venderTable.APPROVESTATUS = 3;
                        //venderTable.ISACTIVE = 0;
                        venderTable.UPDATEBY = UpdateBy;
                        venderTable.UPDATEDATE = UpdateDate;
                        venderTable.PRODTYPEDETAIL = ProductTypeDetail;
                        venderTable.APPROVEMASTERID = approvemasterid;
                        venderTable.VENDPROCESSID = vendprocessid;
                        venderTable.DATAAREAID = DataAreaId;
                        ...
                        var VenderLogDocByRequestID = await unitOfWork.VenderControl.GetVenderLogDocByRequestID(id);
                        var VenderLogDocFilter = VenderLogDocByRequestID.Select(s => s.ID);

                        foreach (var logdocfilter in VenderLogDocFilter)
                        {
                            var vendlogdoc_inactive = await vendLogdocRepo2.GetAsync(logdocfilter);
                            vendlogdoc_inactive.ISACTIVE = 0;

                            await vendLogdocRepo2.UpdateAsync(vendlogdoc_inactive);
                        }
                        ...
                        foreach (var i in approveTransLevel1)
                        {
                            ...
                            if (venderTable.VENDCODE_AX != null)
                            {
                                ...
                                {
                                    var sendRevisemail = _emailService.SendEmail(
                                      $"Subject : {venderTable.REQUESTCODE} / Release / ร้องขอเพื่อสร้างผู้ขายรายใหม่ในระบบ / {vendercode}",
                                      $@"
                                ...
                                else
                                {
                                    ...
                                    var sendRevisemail = _emailService.SendEmail(
                                      $"Subject : {venderTable.REQUESTCODE} {subject} {venderTable.VENDCODE_AX} {venderTable.ISACTIVE} ",
                                            $@"
                                            <b>{body} {venderTable.VENDCODE_AX} </b> <br/>
                                            <b> ผู้ร้องขอ  </b> {EmpFullName}<br/><br/>
                                            <b> รหัสผู้ขาย </b> {venderTable.VENDCODE_AX}<br/>
                            ...
                            else
                            {
                                ...
                                var AppMap = AppMapALL.Where(x => x.APPROVEMASTERID == approvemasterid).Select(s => s.APPROVEGROUPID).FirstOrDefault();
                                ...
                                var sendRevisemail = _emailService.SendEmail(
                                      $"Subject : {venderTable.REQUESTCODE} / Create {AppGroup.DESCRIPTION} / ร้องขอเพื่อสร้างผู้ขายรายใหม่ในระบบ / {vendercode}",
                                      $@"
                                            <b> ร้องขอเพื่อสร้างผู้ขายรายใหม่ในระบบ {vendercode} </b><br/>
                                            <b> ผู้ร้องขอ  </b> {EmpFullName}<br/><br/>
                                            <b> รหัสผู้ขาย </b> {vendercode}<br/>
                    ...
                    AlertSuccess = "Revise Vendor Success.";
                    return Redirect($"/Vender");
                }
            }
            catch (Exception ex)
        ...
        public async Task<IActionResult> OnGetUpdateFileAsync(int Fileid)
        {
            try
            {
                using (var unitOfWork = new UnitOfWork(_databaseContext.GetConnection()))
                {
                    ...
                    return Redirect("/Vender/" + ReqCode + "/Revise");
